...
export default function LyricsSheet({ song, onClose, onDelete, onEdit }) {
  ...
  return (
    <BottomSheet open onClose={onClose}
      title={song.title}
      subtitle={song.author || 'Song details'}
      ...
      {(song.voiceMemo?.url || song.attachment?.url) && (
        <div className="setlist-submission-media" style={{ marginBottom: 16, marginTop: 0 }}>
          <span className="form-label">Song media</span>
          {song.voiceMemo?.url && (
            <div className="setlist-media-item">
              <strong>Voice memo</strong>
              <audio controls preload="metadata" src={song.voiceMemo.url} />
            </div>
          )}
          {song.attachment?.url && (
            <a className="setlist-media-download" href={song.attachment.url} target="_blank" rel="noreferrer">
              ⌁ {song.attachment.name || 'Download attachment'}
            </a>
          )}
        </div>
      )}
